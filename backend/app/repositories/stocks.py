from sqlalchemy import select
from sqlalchemy.orm import Session

from backend.app.db.models import Stock, StockPrice


class StockRepository:
    def __init__(self, db: Session):
        self.db = db

    def get_or_create(self, symbol: str) -> Stock:
        normalized = symbol.upper().strip()
        stock = self.db.scalar(select(Stock).where(Stock.symbol == normalized))
        if stock:
            return stock
        stock = Stock(symbol=normalized)
        self.db.add(stock)
        self.db.flush()
        return stock

    def list_symbols(self) -> list[str]:
        return list(self.db.scalars(select(Stock.symbol).order_by(Stock.symbol)))

    def upsert_price(self, stock: Stock, row: dict) -> None:
        existing = self.db.scalar(
            select(StockPrice).where(
                StockPrice.stock_id == stock.id, StockPrice.trade_date == row["Date"]
            )
        )
        payload = {
            "open": row["Open"],
            "high": row["High"],
            "low": row["Low"],
            "close": row["Close"],
            "volume": int(row["Volume"]),
            "delivery_qty": int(row["DeliveryQty"]),
            "delivery_percent": float(row["DeliveryPercent"]),
        }
        if existing:
            for key, value in payload.items():
                setattr(existing, key, value)
            return
        self.db.add(StockPrice(stock_id=stock.id, trade_date=row["Date"], **payload))
